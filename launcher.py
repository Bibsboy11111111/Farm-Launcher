...
def fetch_latest_release() -> tuple[dict | None, str]:
    ...
    headers = {
        ...
        "User-Agent": "BibsLauncher/1.0",
        "X-GitHub-Api-Version": "2022-11-28",
    }
    # Optional personal access token to bypass unauthenticated rate limits
    token = os.environ.get("GITHUB_TOKEN", os.environ.get("GITHUB_PAT", "")).strip()
    if token:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_err = f"GitHub returned HTTP {e.code}"
            if e.code == 404:
                return None, "No releases found on GitHub. Publish a release first."
            if e.code == 403 and attempt < 3:
                print(f"[LAUNCHER] GitHub 403 on attempt {attempt}, retrying...")
            ...
            if "CERTIFICATE_VERIFY_FAILED" in err_str or "SSL" in err_str:
                return None, f"SSL certificate error. Update Python or install certificates: {e}"
            last_err = err_str
            if attempt < 3:
                time.sleep(2 ** attempt)
                continue
...
class LauncherGUI:
    ...
    def _worker(self):
        ...
        if remote_tag == local_version and local_exe:
